...
export default function Vegetables() {
  ...
  const fallbackVegetables = [
    {
      ...
      rating: 4.8,
      reviewCount: 150,
      sellerName: "Nông dân Đồng Nai",
      specifications: { 'Xuất xứ': 'Đồng Nai', 'Hữu cơ': 'Có' }
    },
    {
      id: 2,
      name: "Cà Chua Cherry",
      ...
      rating: 4.9,
      reviewCount: 89,
      sellerName: "Vườn Lâm Đồng",
      specifications: { 'Xuất xứ': 'Lâm Đồng', 'Hữu cơ': 'Có' }
    },
    {
      id: 3,
      name: "Xà Lách Xoăn",
      ...
      rating: 4.7,
      reviewCount: 203,
      sellerName: "Nông trại Hà Nội",
      specifications: { 'Xuất xứ': 'Hà Nội' }
    },
    {
      id: 4,
      name: "Cải Thảo",
      ...
      rating: 4.6,
      reviewCount: 76,
      sellerName: "Trang trại Bắc Ninh",
      specifications: { 'Xuất xứ': 'Bắc Ninh', 'Hữu cơ': 'Có' }
    },
    {
      id: 5,
      name: "Củ Cải Trắng",
      ...
      rating: 4.5,
      reviewCount: 134,
      sellerName: "Nông dân Thanh Hóa",
      specifications: { 'Xuất xứ': 'Thanh Hóa' }
    },
    {
      id: 6,
      name: "Bông Cải Xanh",
      ...
      rating: 4.8,
      reviewCount: 98,
      sellerName: "Vườn Đà Lạt",
      specifications: { 'Xuất xứ': 'Đà Lạt', 'Hữu cơ': 'Có' }
    }
  ];
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        // Filter only vegetables using consistent category matcher
        const vegetableProducts = allProducts.filter(product => {
          const categoryMatch = isProductInCategory(product.category, 'Rau củ');
          if (!categoryMatch) return false;
          
          // Additional safety check: exclude if product name contains tool/machine keywords
          const productNameLower = (product.name || '').toLowerCase();
          const exclusionKeywords = ['máy', 'dụng cụ', 'tool', 'machine', 'thiết bị', 'máy cày', 'máy phun', 'máy cắt', 'cuốc', 'xẻng'];
        ...
        if (vegetableProducts.length > 0) {
          ...
          setFilteredProducts(vegetableProducts);
        } else {
          setProducts(fallbackVegetables as any);
          setFilteredProducts(fallbackVegetables as any);
        }
      } catch (error) {
  ...
  const getPriceRangeLabel = (): string => {
    ...
    if (priceRange[0] === 0 && priceRange[1] === 30000) return "under-30k";
    if (priceRange[0] === 30000 && priceRange[1] === 60000) return "30k-60k";
    if (priceRange[0] === 60000 && priceRange[1] === 100000) return "60k-100k";
    if (priceRange[0] === 100000 && priceRange[1] === 200000) return "100k-200k";
    if (priceRange[0] === 200000 && priceRange[1] === maxPrice) return "over-200k";
    return "all";
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Rau Củ Tươi Ngon</h1>
          <p className="text-muted-foreground">
            Khám phá bộ sưu tập rau củ tươi ngon, sạch và an toàn từ các trang trại uy tín
          </p>
        </div>
                ...
                <div>
                  ...
                  <Select value={getPriceRangeLabel()} onValueChange={setPriceRangeFromLabel}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="under-30k">Dưới ₫30k</SelectItem>
